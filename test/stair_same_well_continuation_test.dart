...
void main() {
  group('Same-well stair continuation', () {
    test('hasSameWellNextStair detects same-position consecutive stairs', () {
      ...
      expect(
        nav.hasSameWellNextStair(section12),
        isTrue,
    ...
    test(
      'hasSameWellNextStair returns false for different-position stairs',
      () {
        final b = makeBuilding();
    ...
    test(
      'clearStairLockForSameWell clears exitAreas, phase, previousPoint, and unlocks next stair',
      () {
        final b = makeBuilding();
        final s12 = upStair('s12', 1, 2);
        final s23 = upStair('s23', 2, 3);
        ...
        expect(
          ...
          isNull,
          reason:
              'previousPoint must be reset for fresh source-boundary detection',
        );
        expect(
          nav.lockedSections.contains('s23'),
          isFalse,
          reason: 'next stair must be unlocked so it can be armed',
        );
        expect(
          nav.lockedSections.contains('s12'),
          isFalse,
          reason:
              'completed stair lock is also cleared (only matters for same-floor re-entry)',
        );
      },
    );

    test(
      '1→2→3 continuous progression: findRouteTowardFloor returns legs on each floor',
      () {
        final b = makeBuilding();
        final s12 = upStair('s12', 1, 2);
        final s23 = upStair('s23', 2, 3);
    ...
    test(
      '3→2→1 downward progression: findRouteTowardFloor returns legs on each floor',
      () {
        final b = makeBuilding();
    ...
    test(
      'wrong stair (source != currentFloor) does not instantly change floors',
      () {
        final b = makeBuilding();
        final s32 = downStair('s32', 3, 2);
        final s21 = downStair('s21', 2, 1);
    ...
    test(
      'route entry point matches stair entrance that transition detector accepts',
      () {
        final b = makeBuilding();
        final s12 = upStair('s12', 1, 2);
        final s23 = upStair('s23', 2, 3);
    ...
    test('downward same-well detection works for DOWN stairs', () {
      ...
      expect(
        nav.hasSameWellNextStair(section43),
        isTrue,
    ...
    test('after same-well completion, next stair arms without re-entry', () {
      ...
      expect(
        changed,
        isFalse,
        reason: 'floor should not change until player walks through',
      );
      expect(
        nav.transition,
        isNotNull,
    ...
    test(
      'after same-well completion, anti-bounce still prevents returning to completed stair',
      () {
        final b = makeBuilding();
        final s43 = downStair('s43', 4, 3);
        final s32 = downStair('s32', 3, 2);
        ...
        expect(
          nav.lockedSections.contains('s43'),
          isTrue,
        ...
        expect(
          nav.lockedSections.contains('s32'),
          isFalse,
